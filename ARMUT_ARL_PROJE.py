...
df = pd.read_csv("5.Hafta/ArmutARL/armut_data.csv")
df.head()
df.describe().T
df.shape
...
df["CombinedId"] = [str(row[1]) + "_" + str(row[2]) for row in df.values]
df.head()
...
#format=%Y-%m değeri yıl-ay olmak için kullandım.
#dt.to_period ("M") ile aylık periyotlara böldüm.
...
def create_invoice_service_df(df):
    return df.groupby(['ID', 'CombinedId'])['CategoryId'].count().unstack().fillna(0). \
        applymap(lambda x: 1 if x > 0 else 0)

invoice_service_df = create_invoice_service_df(df)
invoice_service_df.head()
# Adım 2: Birliktelik kurallarını oluşturunuz.
frequent_itemsets = apriori(invoice_service_df,
                            min_support=0.01,
                            use_colnames=True)
...
def create_rules(dataframe):
    frequent_itemsets = apriori(invoice_service_df, min_support=0.01, use_colnames=True)
    rules = association_rules(frequent_itemsets, metric="support", min_threshold=0.01)

rules[(rules["support"] > 0.05) & (rules["confidence"] > 0.1) & (rules["lift"] > 2).sort.values("confidence", ascending=False)]
rules = create_rules(invoice_service_df)
