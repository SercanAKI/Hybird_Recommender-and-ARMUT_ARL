...
# Adım 3: Herbir film için toplam kaç kişinin oy kullandığını hesaplayınız.Toplam oy kullanılma sayısı 10000'un
# altında olan filmleri veri setinden çıkarınız.
...
user_movie_count = movies_watched_df.T.notnull().sum() #okunabilir bir hal için transpoz alıyoruz
user_movie_count = user_movie_count.reset_index() #indexten çıkarmak için
#user_movie_count.drop("index", inplace=True, axis=1)
...
#final_df1 = movies_watched_df.loc[movies_watched_df.index.isin(users_same_movies)]
...
#user ıd 2 yı user ıd olarak değiştir
...
# içeren recommendation_df adında yeni bir
...
user = 50485
user_rating = rating[(rating['userId'] == user)].copy() #ayrı bi nesne gibi anladığından google da kopya gibi davranı buldum
